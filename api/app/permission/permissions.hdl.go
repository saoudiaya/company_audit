...
func (db Database) NewPermission(ctx *gin.Context) {
	...
	// check the action passed by the utilisateur
	...
	// check if role nom is empty
	if role.Nom == "" {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": "role nom is invalid"})
		return
	}
...
// search permissions from database
func (db Database) SearchPermissions(ctx *gin.Context) {
	...
	// search permissions
	permissions, err := SearchPermissions(db.DB, permission)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
		return
	}
